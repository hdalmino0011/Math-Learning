...
export const TableSelectScreen: React.FC<TableSelectScreenProps> = ({
  ...
  return (
    <div className="min-h-screen w-full flex flex-col p-4 md:p-6 bg-gradient-to-b from-sky-100 via-sky-50 to-emerald-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      ...
      <header className="w-full max-w-5xl mx-auto flex items-center justify-between pb-4 border-b border-slate-300 dark:border-slate-800">
        <button
          onClick={() => {
            soundManager.playTap();
            onBackToHome();
          }}
          className="p-3 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 rounded-2xl shadow-md border-2 border-slate-300 dark:border-slate-700 hover:scale-105 active:scale-95 transition-all flex items-center gap-2 font-black text-sm cursor-pointer"
        ...
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-lg md:text-xl font-black text-slate-950 dark:text-amber-300">
            {currentInfo.title}
          </span>
        </div>
      </header>
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto flex flex-col items-center justify-center py-6">
        <div className="text-center mb-8">
          <h2 className="text-3xl md:text-4xl font-black text-slate-950 dark:text-slate-100 font-['Fredoka',sans-serif]">
            Choose a Times Table
          </h2>
          <p className="text-sm md:text-base font-black text-slate-700 dark:text-slate-300 mt-1">
            {currentInfo.desc}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border-4 border-emerald-400 dark:border-emerald-700 shadow-xl flex flex-col">
            <div className="flex items-center gap-2 mb-4 pb-2 border-b border-emerald-100 dark:border-slate-700">
              ...
              <div>
                <h3 className="text-lg font-black text-emerald-900 dark:text-emerald-300 flex items-center gap-1.5">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-black text-slate-950 dark:text-white group-hover:text-white text-base">
                          {num} × Table
                        </div>
                        <div className="text-[11px] font-black text-emerald-800 dark:text-emerald-400 group-hover:text-emerald-100">
                          1 to 10
                        </div>
                      </div>
                    </div>
                    ...
                    {masteryCount > 0 ? (
                      <div className="flex items-center gap-1 text-amber-900 dark:text-amber-300 text-xs font-black bg-amber-200 dark:bg-slate-900 px-2.5 py-1 rounded-full border border-amber-400">
                        <StarVector size={14} fill="#f59e0b" />
                        <span>{masteryCount}</span>
                      </div>
                    ) : (
                      <span className="text-emerald-600 group-hover:text-white font-black text-lg">
                        ›
                      </span>
                    )}
                  </button>
          ...
          {/* Tricky Tables Group */}
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border-4 border-amber-400 dark:border-amber-600 shadow-xl flex flex-col">
            <div className="flex items-center gap-2 mb-4 pb-2 border-b border-amber-100 dark:border-slate-700">
              ...
              <div>
                <h3 className="text-lg font-black text-amber-950 dark:text-amber-300 flex items-center gap-1.5">
              ...
              {TRICKY_TABLES.map((num) => {
                ...
                return (
                  <button
                    key={num}
                    onClick={() => handleTablePick(num)}
                    className="group relative flex items-center justify-between p-3.5 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-slate-700 dark:to-slate-800 hover:from-amber-500 hover:to-orange-500 hover:text-white rounded-2xl border-2 border-amber-300 dark:border-slate-600 shadow-sm hover:shadow-md hover:scale-[1.02] active:scale-95 transition-all text-left cursor-pointer"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-[11px] font-black text-amber-800 dark:text-amber-400 group-hover:text-amber-100">
                          1 to 10
                        </div>
                      </div>
                    </div>
                    ...
                    {masteryCount > 0 ? (
                      ...
                      <span className="text-amber-600 group-hover:text-white font-black text-lg">
                        ›
                      </span>
                    )}
                  </button>
